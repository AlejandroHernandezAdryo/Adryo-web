 ...
 */
        public function index() {
            ...
            if ($this->request->is('post')){
                $cuenta = array('Inmueble.cuenta_id'=>$this->Session->read('CuentaUsuario.Cuenta.id'), 'Inmueble.id NOT IN (SELECT inmueble_id FROM desarrollo_inmuebles)');
...
//			$this->request->data['Inmueble']['luz_agua'] = "";
			...
			if ($this->Inmueble->save($this->request->data)) {
                ...
                $this->Session->setFlash('Se han creado las correctamente la propiedad.', 'default', array(), 'm_success');
				...
				$this->Session->setFlash('', 'default', array(), 'success');
                $this->Session->setFlash('No se ha podido crear el inmueble, favor de intentarlo nuevamente <br> Gracias.', 'default', array(), 'm_success');
			}
			//echo var_dump($this->request->data);
		}
                $opcionadors = $this->User->find('list',array('conditions'=>array("User.id IN (SELECT user_id FROM cuentas_users WHERE opcionador = 1 AND cuenta_id = ".$this->Session->read('CuentaUsuario.CuentasUser.cuenta_id').")"),'order'=>'User.nombre_completo ASC'));
 ...
 */
    ...
    *******************************************************/
    ...
    public function get_inmueble_detalle( $inmueble_id = null ) {
        ...
        //$inmueble['Inmueble']['comentarios150']     = substr(strip_tags(str_replace($limpieza, "", $inmueble['Inmueble']['comentarios'])), 0, 150).'...';
    ...
    public function busqueda_avanzada( $data = null ) {

        // Filtro para la busqueda de inmuebles.

        $resp = array(
            'mensaje' => 'Hola desde busqueda avanzada',
            'bandera' => true
        );
