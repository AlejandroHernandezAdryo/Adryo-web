<style>
    #grafica_temporales_linea_contacto{
        width: 100%;
        height: 500px;
    }
...
<div class="card">
    ...
    <div class="card-block">
      <div class="row">
        <div class="col-sm-12" >
          <div id="grafica_temporales_linea_contacto" class="grafica"></div>
        </div>
        <div class="col-sm-12 m-t-35">
          <small id="leads_temporales_linea_contacto_periodo_tiempo"></small>
        </div>
      </div>
    </div>
</div>
...
<script>
  // Se mandan a llamar los datos para la grafica
    function graficaTemporalesLineaContacto( rangoFechas, cuentaId, desarrolloId ,asesorId ){
        $.ajax({
        type: "POST",
        url: '<?php echo Router::url(array("controller" => "leads", "action" => "grafica_temporales_linea_contacto")); ?>',
        cache: false,
        ...
        success: function ( response ) {
            ...
            document.getElementById("leads_temporales_linea_contacto_periodo_tiempo").innerHTML =rangoFechas;
            if (maximoLead==0) {
                maximoLead=10
            }
            ...
            drawTemporalesLineaContacto( response,maximaInversion,InversionTotal,cantidadTotal, maximoLead );
    ...
    function drawTemporalesLineaContacto(response,maximaInversion,InversionTotal,cantidadTotal, maximoLead){
		am5.ready(function () {
			var root = am5.Root.new("grafica_temporales_linea_contacto");
			root.setThemes([
			...
			yAxis.children.unshift(
				am5.Label.new(root, {
					rotation: -90,
					text: "Clientes inactivos temporales",
					y: am5.p50,
					centerX: am5.p50
				})
			);
			...
			var series = chart.series.push(am5xy.ColumnSeries.new(root, {
				name                  : `Clientes inactivos temporales: ${cantidadTotal}`,
				xAxis                 : xAxis,
				yAxis                 : yAxis,
				valueYField           : "leads",
			...
			series.set("fill", am5.color("<?= $this->Session->read('colores.InactivoTemporal')?>")); 
