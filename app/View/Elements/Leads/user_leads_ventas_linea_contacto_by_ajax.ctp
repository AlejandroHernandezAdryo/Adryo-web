<style>
    #grafica_ventas_linea_contacto_user{
        width: 100%;
        height: 500px;
    }
...
<div class="card">
    ...
    <div class="card-block" style="width: 100%;">
      <div class="row">
        <div class="col-sm-12" >
          <div id="grafica_ventas_linea_contacto_user" class="grafica"></div>
        </div>
...
<script>
  ...
  function gradicaLeadsVentasInversionUSER( rangoFechas, cuentaId, desarrolloId ,asesorId ){
    
    $.ajax({
        type: "POST",
        url: '<?php echo Router::url(array("controller" => "leads", "action" => "grafica_ventas_linea_contacto_user_view")); ?>',
        cache: false,
  ...
  function drawGraficaVentasLineaContacto( response,maximaInversion,InversionTotal,cantidadTotal,ventaTotal, maximoLead ) {
    am5.ready(function () {
			var root = am5.Root.new("grafica_ventas_linea_contacto_user");
			root.setThemes([
				am5themes_Animated.new(root)
			]);
			root.interfaceColors.set("grid", am5.color("<?= $this->Session->read('colores.Cliente')?>"));
			...
			// series1.set("fill", am5.color('#63BE38')); 
