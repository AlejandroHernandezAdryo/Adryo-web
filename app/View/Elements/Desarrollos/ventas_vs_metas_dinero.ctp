<?php 
    ...
    foreach ($kpi_arreglo as $periodo): 
        $total_venta        += $periodo['ventas_monto']/1000000;
        $objetivo_acumulado += $periodo['objetivo_monto']/1000000;
        $titulo_total_venta += number_format($periodo['ventas_monto']/1000000,1);
    endforeach;
...
<div class="card">
    ...
    <div class="card-block" style="width: 100%; height: 350px">
        <div class="row">
            <div class="col-sm-12">
                <div id="metas_ventas_desarrollo_2" style="width: 100%; min-height: 300px;"></div>
            </div>
        </div>
        <div class="row">
...
function drawVentasMetas(){
    var data = google.visualization.arrayToDataTable([
        ['Month', 'Meta <?= number_format($objetivo_acumulado,0)?> (MDP)', {role: 'annotation'}, 'Ventas <?= $titulo_total_venta ?> (MDP)', {role: 'annotation'}, 'Cumplido <?= number_format($porcentaje*100,0)?>%'],
        ...
        <?php else: ?>
            <?php 
                foreach ($kpi_arreglo as $periodo):
                    $porcentaje = 0;
                    $venta_mes = number_format($periodo['ventas_monto']/1000000,1);
                    $objetivo_mes = number_format($periodo['objetivo_monto']/1000000,1);
                    if($objetivo_mes == 0){
                        $porcentaje = 0;
                    }else{
            ...
            ?>
            ['<?= $periodo['periodo']?>', <?= $objetivo_mes?>,<?= $objetivo_mes?>, <?= $venta_mes?>, <?= $venta_mes ?>, <?= $porcentaje ?> ],
            <?php endforeach;?>
        <?php endif; ?>
